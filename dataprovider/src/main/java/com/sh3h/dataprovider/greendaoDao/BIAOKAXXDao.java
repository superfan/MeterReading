 ...
 */
public class BIAOKAXXDao extends AbstractDao<BIAOKAXX, Void> {
    ...
    public static class Properties {
        ...
        public final static Property D_DUORENKJZ = new Property(74, int.class, "D_DUORENKJZ", false, "D_DUORENKJZ");
        ...
        public final static Property I_XIUGAIBZ = new Property(81, int.class, "I_XIUGAIBZ", false, "I_XIUGAIBZ");
        public final static Property I_SHANGCHUANBZ = new Property(82, int.class, "I_SHANGCHUANBZ", false, "I_SHANGCHUANBZ");
    }
    ...
    protected void bindValues(SQLiteStatement stmt, BIAOKAXX entity) {
        ...
        int D_DUORENKJZ = entity.getD_DUORENKJZ();
        //if (D_DUORENKJZ != null) {
        ...
        int I_XIUGAIBZ = entity.getI_XIUGAIBZ();
        stmt.bindLong(82, I_XIUGAIBZ);

        int I_SHANGCHUANBZ = entity.getI_SHANGCHUANBZ();
        stmt.bindLong(83, I_SHANGCHUANBZ);
    }
    ...
    public BIAOKAXX readEntity(Cursor cursor, int offset) {
        BIAOKAXX entity = new BIAOKAXX( //
                ...
                cursor.isNull(offset + 16) ? 0 : cursor.getInt(offset + 16), // D_LIHU
                ...
                cursor.isNull(offset + 36) ? 0 : cursor.getInt(offset + 36), // D_ZHUANGBIAORQ
                cursor.isNull(offset + 37) ? 0 : cursor.getInt(offset + 37), // D_HUANBIAORQ
                ...
                cursor.isNull(offset + 74) ? 0 : cursor.getInt(offset + 74), // D_DUORENKJZ
                ...
                cursor.isNull(offset + 81) ? 0 : cursor.getInt(offset + 81), // I_XIUGAIBZ
                cursor.isNull(offset + 82) ? 0 : cursor.getInt(offset + 82) // I_SHANGCHUANBZ
        );
        return entity;
    }
    ...
    public void readEntity(Cursor cursor, BIAOKAXX entity, int offset) {
        ...
        entity.setD_LIHU(cursor.isNull(offset + 16) ? 0 : cursor.getInt(offset + 16));
        ...
        entity.setD_ZHUANGBIAORQ(cursor.isNull(offset + 36) ? 0 : cursor.getInt(offset + 36));
        entity.setD_HUANBIAORQ(cursor.isNull(offset + 37) ? 0 : cursor.getInt(offset + 37));
        ...
        entity.setD_DUORENKJZ(cursor.isNull(offset + 74) ? 0 : cursor.getInt(offset + 74));
        ...
        entity.setI_XIUGAIBZ(cursor.isNull(offset + 81) ? 0 : cursor.getInt(offset + 81));
        entity.setI_SHANGCHUANBZ(cursor.isNull(offset + 82) ? 0 : cursor.getInt(offset + 82));
    }
     ...
     */
    @Override
    protected Void updateKeyAfterInsert(BIAOKAXX entity, long rowId) {
     ...
     */
    @Override
    public Void getKey(BIAOKAXX entity) {
        return null;
    }
    ...
    public void delectBiaoKaXX(String CIDAll) {
        ...
        DeleteQuery<BIAOKAXX> bd = qb.where
                (Properties.S_CID.in((Object[]) str)).buildDelete();
    ...
    public boolean updateBiaoKaXXYanChiDialog(String cid, String kouJing,
        ...
        biaokaxx.setD_HUANBIAORQ((int) huanBiaoRQ);
    ...
    public void insertOrUpdate(List<BIAOKAXX> biaoKaXXList) {
        ...
        for (int i = 0; i < biaoKaXXList.size(); i++) {
            insertOrReplace(biaoKaXXList.get(i));
        }
    }
...
//                list.add((BIAOKAXX)cursor);
    ...
    public List<BIAOKAXX> getCombinedQuery(ConditionInfo info){
        ...
        if (info.getmRWBHList() != null && info.getmRWBHList().size() > 0) {
            for (String chaoBiaoRW : info.getmRWBHList()) {
                builder.append("'" + chaoBiaoRW + "',");
            }
        }

        String sql = "select * from KG_BIAOKAXX BK where BK.S_CH in ( "+ builder.toString().substring(0, builder.length() - 1) + " ) ";

        StringBuilder sbuff = new StringBuilder();
        if(!TextUtils.isEmpty(info.get_cid())){
            sbuff.append(" AND TRIM(BK.S_CID) = '" + info.get_cid() + "'");
        }
        if(!TextUtils.isEmpty(info.get_cebenhao())){
            sbuff.append(" AND TRIM(BK.S_CH) = '" + info.get_cebenhao() + "'");
        }
        if(!TextUtils.isEmpty(info.get_huming())){
            sbuff.append(" AND TRIM(BK.S_KEHUMC) like '%" + info.get_huming().trim() + "%'");
        }
        if(!TextUtils.isEmpty(info.get_dizhi())){
            sbuff.append(" AND TRIM(BK.S_DIZHI) like '%" + info.get_dizhi().trim() + "%'");
        }
        if(!TextUtils.isEmpty(info.get_lianxidh())){
            sbuff.append(" AND TRIM(BK.S_LIANXIDH) like '%" + info.get_lianxidh().trim() + "%'");
        }
        if(!TextUtils.isEmpty(info.get_jianhao())){
            sbuff.append(" AND TRIM(BK.S_JIANHAO) like '%" + info.get_jianhao().trim()+ "%'");
        }
        if(!TextUtils.isEmpty(info.get_biaohao())){
            sbuff.append(" AND TRIM(BK.S_SHUIBIAOGYH) like '%" + info.get_biaohao().trim() + "%'");
        }
        if(info.get_koujinmin() < 999999998){
            sbuff.append(" AND cast(TRIM(BK.S_KOUJINGMC) as integer) >= "+ info.get_koujinmin());
        }
        if(info.get_koujinmax()> -1){
            sbuff.append(" AND cast(TRIM(BK.S_KOUJINGMC) as integer) <= "+ info.get_koujinmax());
        }
        if(info.get_qianfeibs() > -1){
            sbuff.append(" AND BK.[I_QianFeiZBS] >= " + info.get_qianfeibs());
        }
        if(info.get_qianfeije() > -1){
            sbuff.append(" AND BK.[N_QIANFEIZJE] >= " + info.get_qianfeije());
        }
        if(info.get_huanbiaorq() > 0){
            sbuff.append(" AND BK.[D_HUANBIAORQ] >= " + info.get_huanbiaorq());
        }
   ...
   /*     QueryBuilder qb = this.queryBuilder();
        ...
        qb.where(Properties.S_CID.eq(info.get_cid()),Properties.S_CH.in(info.getmRWBHList())
                //    Properties.S_CH.eq(info.get_cebenhao()),
                //    Properties.S_KeHuMC.like(info.get_huming()),
                //    Properties.S_DiZhi.like(info.get_dizhi()),
                //    Properties.S_LIANXIDH.like(info.get_lianxidh()),
                //    Properties.S_JianHao.like(info.get_jianhao()),
                //    Properties.S_SHUIBIAOGYH.like(info.get_biaohao()),
                //    Properties.S_KOUJINGMC.ge(String.valueOf(info.get_koujinmin()))
                //    Properties.S_KOUJINGMC.le(String.valueOf(info.get_koujinmax())),
                //    Properties.I_QianFeiZBS.ge(info.get_qianfeibs()),
                //    Properties.N_QianFeiZJE.ge(info.get_qianfeije()),
                //    Properties.D_HUANBIAORQ.ge(info.get_huanbiaorq())
        );
        qb.orderAsc(Properties.S_CID);
    ...
    private BIAOKAXX analyze(Cursor cursor) {
        ...
        biaoKaXX.setD_LIHU(cursor.getInt(cursor
        ...
        biaoKaXX.setD_ZHUANGBIAORQ(cursor.getInt(cursor
                .getColumnIndex(BiaoKaXXColumns.D_ZHUANGBIAORQ)));
        biaoKaXX.setD_HUANBIAORQ(cursor.getInt(cursor
        ...
        biaoKaXX.setD_DUORENKJZ(cursor.getInt(cursor
